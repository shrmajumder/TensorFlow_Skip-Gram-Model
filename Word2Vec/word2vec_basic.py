...
def generate_batch(data, batch_size, num_skips, skip_window):
 ...
 """  
  ...
  for i in range(batch_size // num_skips):
    ...
    middle=int((i+end)/2)
    target = data[middle]  # target label at the center of the buffer
    list.append(middle)
    for j in range(num_skips):
        while middle in list:
            middle = random.randint(i,end)
        #print (list)
        list.append(middle)
        #print ("Hi",list)
        batch[i * num_skips + j] = target
        labels[i * num_skips + j,0] = data[middle]
